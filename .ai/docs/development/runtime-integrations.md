...
- `make infra-curated-pipeline` readiness now requires: `iceberg_rest` healthy, `trino` healthy, and `trino_curated_init` exit code `0`.

## Curated Transform SQL Packaging
- Remaining curated transform SQL is task-scoped and embedded in the task modules that execute it:
