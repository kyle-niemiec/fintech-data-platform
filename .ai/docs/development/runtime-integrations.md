...
- The footer session countdown (`apps/ui/src/components/layout/SessionTimer.tsx` + `apps/ui/src/hooks/useSessionTimer.ts`) reads `import.meta.env.VITE_LAUNCHER_API_URL` and polls the launcher control `GET /status` every 60 s for `stop_scheduled_at`. The component subscribes to the shared 1-second `useNow` ticker to render `mm:ss` remaining; it returns `null` when the URL is unset, when `instance_state` is not `running`/`pending`, when `stop_scheduled_at` is absent, or once the countdown reaches zero. Build-arg plumbing matches the other `VITE_*` vars: Dockerfile `ARG/ENV VITE_LAUNCHER_API_URL` → compose `args.VITE_LAUNCHER_API_URL` → `generate_env.sh LAUNCHER_API_URL` → `env_templating.EnvTemplateRenderer.launcher_api_url`. The release workflow resolves the Lambda Function URL via `cloudformation describe-stacks` and forwards it through `ssm_release_deploy.sh` → `ec2_deploy_release.sh`.
