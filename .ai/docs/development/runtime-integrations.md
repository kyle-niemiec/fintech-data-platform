...
- The footer session countdown (`apps/ui/src/components/layout/SessionTimer.tsx` + `apps/ui/src/hooks/useSessionTimer.ts`) reads `import.meta.env.VITE_LAUNCHER_API_URL` and polls the launcher control `GET /status` for `stop_scheduled_at`. The component subscribes to the shared 1-second `useNow` ticker to render `mm:ss` remaining; it returns `null` when the URL is unset, when `stop_scheduled_at` is absent, or once the countdown reaches zero. Build-arg plumbing matches the other `VITE_*` vars: Dockerfile `ARG/ENV VITE_LAUNCHER_API_URL` → compose `args.VITE_LAUNCHER_API_URL` → `generate_env.sh LAUNCHER_API_URL` → `env_templating.EnvTemplateRenderer.launcher_api_url`. The release workflow resolves the Lambda Function URL via `cloudformation describe-stacks` and forwards it through `ssm_release_deploy.sh` → `ec2_deploy_release.sh`.
- `useSessionTimer` uses an adaptive `refetchInterval`: 60 s during a normal session, 3 s once `instance_state === "stopping"` or once `Date.now()` has passed `stop_scheduled_at`, and `false` (halted) once `instance_state === "stopped"`. It exposes both `stopAt` and the latest `instanceState` so the shutdown overlay can react to the running → stopping → stopped transition without an extra fetch path.
- `apps/ui/src/components/layout/ShutdownOverlay.tsx` is mounted once at the root layout in `App.tsx`. It renders a full-screen modal (`role="dialog"`, spinner + "The server is shutting down" copy) only when the timer has expired AND `instanceState` is non-null and not `"running"`. When `instanceState === "stopped"`, a `useEffect` calls `window.location.replace("/?t=" + Date.now())` to hard-refresh back to `/`; with EC2 down, CloudFront origin failover lands the user on the S3 launcher page.
