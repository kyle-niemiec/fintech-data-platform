...
- Partition names are generated by pg_partman-managed monthly suffixes (for example `event_log_p2026_04` / `alert_event_p2026_04`).
- Partition lifecycle is managed in-database by `pg_partman` with monthly premake horizon and `pg_cron` scheduled maintenance (`SELECT event_store.run_partman_maintenance();`).
- Event-store migrations define template tables for per-partition indexes and register both parents with `partman.create_parent(...)`.
