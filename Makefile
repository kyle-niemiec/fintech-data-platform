...
export KEYCLOAK_URL KEYCLOAK_REALM KEYCLOAK_API_CLIENT_ID KEYCLOAK_API_AUDIENCE KEYCLOAK_SWAGGER_CLIENT_ID
...
help:
	...
	@printf "  infra-pg-up              Start Postgres + MinIO containers\n"
	...
	@printf "  infra-clean              Stop containers and remove local Postgres/MinIO volumes and Terraform state\n"
...
infra-up:
	...
	@printf "  4) make infra-kc-up (wait for this to fully start)\n"
...
infra-pg-up:
	docker compose -f $(COMPOSE_FILE) --env-file $(INFRA_ENV_FILE) up -d postgres minio
